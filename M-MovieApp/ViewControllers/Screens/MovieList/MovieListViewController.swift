...
class MovieListViewController: BaseViewController {
    ...
    private func setupTableCell(_ cell: UpcomingTableCell, at indexPath: IndexPath) {
        cell.data = self.upcomingViewModel?.movies?[indexPath.row]
    }
    ...
    private func routeToDetailViewController(_ movieId: Int?) {
        guard let movieId = movieId else {
            presentAlert(with: "An error occurred.")
            return
        }

        let destination = MovieDetailViewController(movieId)
...
extension MovieListViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let movieId = upcomingViewModel?.movies?[indexPath.row].id
        routeToDetailViewController(movieId)
    }
}
...
extension MovieListViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let movieId = nowPlayingViewModel?.movies?[indexPath.row].id
        routeToDetailViewController(movieId)
    }
}
